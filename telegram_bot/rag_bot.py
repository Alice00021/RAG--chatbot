...
if not API_GATEWAY_URL:
    logger.error("API_GATEWAY_URL не найден в переменных окружения. Проверьте файл .env")
    raise ValueError("API_GATEWAY_URL не найден в переменных окружения. Проверьте файл .env")
...
async def rag_message(message: types.Message, **kwargs):
    ...
    message_data = {
        ...
        'chat_id': chat_id,
        'chat_id_in_telegram': message.chat.id 
    }
